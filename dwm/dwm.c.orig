...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - stw - x) > bh) {
        ...
        if (c->issticky) {
          // possibly can cahnge the name of a sticky client display
...
runautostart(void)
{
	...
	/* first we're gonna check if HOME/dwm exists, and use that by default then 
   * if $XDG_DATA_HOME is set and not empty, use $XDG_DATA_HOME/dwm,
	 * otherwise use ~/.local/share/dwm as autostart script directory
	 */

	xdgdatahome = getenv("XDG_DATA_HOME");
  if (autostart_prefix_dot_config != 0) {

		pathpfx = ecalloc(1, strlen(home) + strlen(".config") + strlen(dwmdir) + 3);
